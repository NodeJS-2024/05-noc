...
describe('Pruebas en FileSystemaDataSource.ts', () => {
  ...
  test('should save a log in all logs logs-all.log', async () => {
  ...
  test('should save a log in all logs logs-all.log and logs-medium.log', async () => {
  ...
  test('should save a log in all logs logs-all.log and logs-high.log', async () => {
  ...
  test('should return all logs', async () => {
    ...
    const logsmeidum = await logDataSource.getLogs(LogSeverityLevel.medium);
    const logsHigh = await logDataSource.getLogs(LogSeverityLevel.high);

    expect(logsLow).toEqual(expect.arrayContaining([ logLow, logMedium, logHigh ]));
    expect(logsmeidum).toEqual(expect.arrayContaining([ logMedium ]));
  ...
  test('should throw an error if severity level is not defined', async () => {
    ...
    // Consideralo un enum de LogSeverityLevel
    const customSeverityLevel = 'SUPER_MEGA_HIGH' as LogSeverityLevel;

    try {
      await logDataSource.getLogs(customSeverityLevel);
      expect(true).toBeFalsy(); // Nunca se debe de ejecutar
    } catch (error) {
